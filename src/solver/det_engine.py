...
def evaluate(
    ...
    **kwargs,
    ...
    if coco_evaluator is not None and "bbox" in iou_types:
        ...
        # 新版 faster-coco-eval
        if hasattr(bbox_eval, "extended_metrics"):
            ext = bbox_eval.extended_metrics
            class_rows = ext.get("class_map", [])

            print("\nPer-class metrics:")
            for row in class_rows:
                print(
                    f"{row['class']:20s} "
                    f"mAP@50:95={row.get('map@50:95', float('nan')):.4f} "
                    f"mAP@50={row.get('map@50', float('nan')):.4f} "
                )

            stats["per_class_metrics"] = class_rows
