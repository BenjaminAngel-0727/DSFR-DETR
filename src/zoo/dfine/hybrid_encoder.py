...
from .frequency_domain import WaveletGuidedP2Enhancer
...
class HybridEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.use_wavelet_p2_enhance = use_wavelet_p2_enhance
        if self.use_wavelet_p2_enhance:
            assert len(in_channels) == 4, "WaveletGuidedP2Enhancer expects 4-level features."
            assert feat_strides[0] == 4 and feat_strides[-1] == 32, \
                "Expected feat_strides like [4, 8, 16, 32]."
            self.wavelet_p2_enhancer = WaveletGuidedP2Enhancer(hidden_dim, act=act)
    ...
    def forward(self, feats, return_intermediates=False):
        ...
        # 新增：P2 wavelet enhancement
        # 当前配置下，proj_feats[0]=P2@160x160, proj_feats[3]=P5@20x20(已过AIFI)
        wavelet_aux = None
        if self.use_wavelet_p2_enhance:
            if return_intermediates:
                proj_feats[0], wavelet_aux = self.wavelet_p2_enhancer(
                    proj_feats[0], proj_feats[3], return_intermediates=True
                )
            else:
                proj_feats[0] = self.wavelet_p2_enhancer(proj_feats[0], proj_feats[3])
